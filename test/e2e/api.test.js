...
describe('API E2E Test Suite', () => {
  ...
  describe('/login', () => {
    it('should receive not authorized when user or password is invalid', async () => {
      ...
      assert.deepStrictEqual( // actual, expected, message
        response,
        expectedBody,
        `response.body should be ${JSON.stringify(expectedBody)}, actual ${JSON.stringify(response)}`
      )
    })

    it('should login successfully given a user and password', async () => {
      const input = {
        user: 'erickwendel',
        password: '123'
      }
      ...
      const response = await result.json()
      assert.ok(response.token.length > 20, `response.token should be a valid jwt tokan, actual: ${response.token}`)
  ...
  describe('/', () => {
    it('should not be allowed to access private data without', async () => {
      const input = {
        headers: {
          authorization: ''
        }
